...
async function AddAccount(req: Request, res: Response<MessageResponse>) {
...
app.get("/bar", csrfProtect,async (req, res: FooResponse) => {
    console.log("Waiting...");
 ...
 * 
 */
app.get("/books",csrfProtect,async (req,res:BookResponse)=>{
let books :Book= await db.all("SELECT * FROM books ");
  res.send(books)
});
// get author list
app.get("/authors",csrfProtect,async (req,res:AuthorResponse)=>{
 let authors : Author = await db.all("SELECT * FROM authors")
  res.send(authors)
})
//get use book id query book info 
app.get("/book", csrfProtect,async (req,res:BookResponse)=>{
    if (!req.query.id) {
...
app.get("/genrebook", csrfProtect,async (req,res:BookResponse)=>{
    if (!req.query.genre) {
...
app.get("/author", csrfProtect,async (req,res:AuthorResponse)=>{
    if (!req.query.id) {
        return res.status(400).json({ error: "id is required" });
    }
...
// insert book data 
app.post("/book",authorize,csrfProtect,async (req, res:BookResponse )=>{
...
// insert author data
app.post("/author",authorize, csrfProtect,async (req,res:AuthorResponse )=>{
    let authors = await db.all("SELECT * FROM authors");
...
app.post("/updateBook",authorize,csrfProtect,async (req, res:BookResponse)=>{
    let books = await db.all("SELECT * FROM books");
    let authors = await db.all("SELECT * FROM authors");
...
// update author data
app.post("/updateAuthor", csrfProtect,async (req, res:AuthorResponse )=>{
...
app.delete("/book/", authorize, csrfProtect,async (req,res:BookResponse)=>{
    if (!req.query.id) {
...
app.delete("/author", csrfProtect,async (req,res:AuthorResponse)=>{
    if (!req.query.id) {
        return res.status(400).json({ error: "delete id is required" });
    }
    let authors = await db.all("SELECT * FROM authors");
...
app.put("/updateBook",authorize,csrfProtect,async (req, res:BookResponse)=>{
    let books = await db.all("SELECT * FROM books");
    let authors = await db.all("SELECT * FROM authors");
    const sql = "UPDATE books set  author_id = ? , title = ? , pub_year = ? , genre = ? where id = ? "
    const {id,author_id,title,pub_year,genre}=req.body
