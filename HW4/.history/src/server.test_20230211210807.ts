...
test("PUT / updatebook author not exist", async ()=>{
   const body ={
    "id":4,
    "author_id":"500",
    "title":"My Fairest Lady",
    ...
    try {
   await axios.put(`${baseUrl}/updateBook`,body)
    } catch (error) {
        // casting needed b/c typescript gives errors "unknown" type
        let errorObj = error as AxiosError;
        // if server never responds, error.response will be undefined
...
test("PUT /Updatebook work ok ", async ()=>{
   const body ={
    "id":"1",
    "author_id":1,
    "title":"My Fairest Lady",
  ...
  console.log(body)
  let result = await axios.put(`${baseUrl}/updateBook`,body)
  expect(result.data).toEqual("OK")

})
test("PUT  / error Update book  id not   exist", async ()=>{
   const body ={
    "id":"300",
    "author_id":1,
    "title":"My Fairest Lady",
