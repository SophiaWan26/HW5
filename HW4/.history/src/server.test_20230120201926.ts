...
test("POST / error Update book  id  exist", async ()=>{
   const body ={
    "id":"10",
    "author_id":1,
    "title":"My Fairest Lady",
...
test("POST / error Updateauthor work bad author exist", async ()=>{
   const body ={
        "id": "1",
        "name": "Figginsworth III",
        "bio": "A traveling gentleman."
...
test("POST / Update author insert work ok", async ()=>{
   const body ={
        "id": "2",
        "name": "Figginsworth III",
        "bio": "A traveling gentleman."
  }
  let result = await axios.post(`${baseUrl}/update`,body)
  expect(result.data).toEqual("OK")
