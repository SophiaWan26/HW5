...
const AddBook =(props:Props)=>{ 
  ...
  const addBookPost=()=>{
    ...
    console.log(data)
 axios.post("http://localhost:3001/book",data).then((res =>{
      ...
      if(res.data.message==="insert id"){
        ...
        window.alert("error => book id is exist, Please re-enter")
        setid("")
        setauthor_id("")
        settitle("")
        setpub_year("")
    ...
    <form>
      ...
      <div>
        ...
        <div>
          ...
          <input type="button" value="add Book" onClick={addBookPost}/>
      </div>
    </form>
  </div>
}
